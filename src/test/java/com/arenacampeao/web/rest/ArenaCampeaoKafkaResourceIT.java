package com.arenacampeao.web.rest;
...
import com.arenacampeao.config.KafkaProperties;
...
class ArenaCampeaoKafkaResourceIT {
    ...
    void setup() {
        ...
        ArenaCampeaoKafkaResource kafkaResource = new ArenaCampeaoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/arena-campeao-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/arena-campeao-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
